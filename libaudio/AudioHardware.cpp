...
status_t AudioHardware::AudioStreamInALSA::open_l()
{
	...
	if (mHardware->mode() == AudioSystem::MODE_IN_CALL) {
		...
		struct mixer_ctl *ctlMic = mixer_get_ctl_by_name(mMixer, "Voice Call Path");
		if (ctlMic != NULL ) {
		    // set proper Mic Path router for voicecall.
		    TRACE_DRIVER_IN(DRV_MIXER_SEL)
		    mixer_ctl_set_enum_by_string(ctlMic , mHardware->getInputRouteFromDevice(mDevices));
		    TRACE_DRIVER_OUT
		}

		ctlMic = mixer_get_ctl_by_name(mMixer, "MIC Path");
		if (ctlMic != NULL ) {
		    // set proper Mic Path router when in call.
		    TRACE_DRIVER_IN(DRV_MIXER_SEL)
		    mixer_ctl_set_enum_by_string(ctlMic , mHardware->getMicPathFromDevice(mDevices));
		    TRACE_DRIVER_OUT
		}
	}
	...
	if (mHardware->mode() != AudioSystem::MODE_IN_CALL) {
		...
		struct mixer_ctl *ctlMic = mixer_get_ctl_by_name(mMixer, "Voice Memo Path");
		if (ctlMic != NULL ) {
...
status_t AudioHardware::AudioStreamInALSA::setParameters(const String8& keyValuePairs)
{
    ...
    mSleepReq = true;
    {
        ...
        if (param.get(String8(INPUT_SOURCE_KEY), source) == NO_ERROR) {
            ...
            mMixer = mHardware->openMixer_l();
	    if (mMixer) {
		mRouteCtl = mixer_get_ctl_by_name(mMixer, "Voice Memo Path");
		if (mRouteCtl != NULL) {
		   mixer_ctl_set_enum_by_string(mRouteCtl,  mHardware->getInputRouteFromDevice(mDevices));
		}
	    }
            mHardware->closeMixer_l();
